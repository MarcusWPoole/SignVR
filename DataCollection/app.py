...
class LeapDataCapture: MonoBehaviour
...
currentFrame;
private
List < float[] > sequence = new
List < float[] > ();
private
const
int
dataPointsPerHand = 204;
private
float[]
entry = new
float[2 * dataPointsPerHand];
public
TextMeshProUGUI
dataDisplayText;
public
...
detectedText;
private
Queue < Func < Task >> asyncQueue = new
Queue < Func < Task >> ();
private
const
int
desiredSequenceLength = 6; // adjust
this
to
the
sequence
...
const
string
baseURL = "http://10.130.2.68:5000"; // server
address
...
{
    Debug.LogError(
        "LeapProvider not set. Please assign it in the inspector or ensure a LeapProvider is present in the scene.");
return;
}
}

leapProvider.OnUpdateFrame += UpdateCurrentFrame; // Subscribe
to
frame
updates
StartCoroutine(CollectDataEveryDelay(0.2
f)); // Start
the
data
collection
...
while (asyncQueue.Count > 0)
    ...
    {
    if (leapProvider != null)
        leapProvider.OnUpdateFrame -= UpdateCurrentFrame; // Unsubscribe
        to
        prevent
        any
        potential
    ...
    Hand
    leftHand = currentFrame.GetHand(Chirality.Left);
    Hand
    rightHand = currentFrame.GetHand(Chirality.Right);

    // Simplified
    hand
    detection
    logging
    detectedText.text = (leftHand != null ? (rightHand != null ? "Both Hands Detected": "Left Hand Detected"): (
                rightHand != null ? "Right Hand Detected": "No Hands Detected"));

    // Process
    the
    data
    for each hand and store it in the entry array
    ...
    {
    sequence.Add(entry.Clone() as float[]); // Using
    Clone
    to
    ensure
    we
    're adding a copy of the entry to the sequence
    Debug.Log($"Sequence size after adding: {sequence.Count}");
    }

    if (sequence.Count > desiredSequenceLength)
    {
    asyncQueue.Enqueue(() = > SendDataToServerAndDisplayResult(new
    List < float[] > (sequence))); // Clone
    the
    current
    sequence and enqueue
    it
    sequence.RemoveAt(0); // Remove
    the
    oldest
    entry
    after
    ...
    {
        float[]
    handData = GetHandData(hand, debugArmData, debugPalmData, debugFingerData);
    ...
    with zeros
        ...
        for (int i = 0; i < data.Length; i++)
            ...
            {
                Bone
                bone = finger.Bone((Bone.BoneType)
                b);
            ...
            float
            CalculateRoll(Vector3
            palmNormal, Vector3
            handDirection)
            {
            Vector3
            normalProjectedOnPlane = Vector3.ProjectOnPlane(palmNormal, handDirection);
            float
            rollAngle = Mathf.Atan2(normalProjectedOnPlane.y, normalProjectedOnPlane.x);
            return rollAngle * Mathf.Rad2Deg;
            }

            float
            CalculatePitch(Vector3
            handDirection)
            {
            return Mathf.Atan2(handDirection.y, Mathf.Sqrt(
                handDirection.x * handDirection.x + handDirection.z * handDirection.z)) * Mathf.Rad2Deg;
            }

            float
            CalculateYaw(Vector3
            handDirection)
            {
            ...
            private async Task
            SendDataToServerAndDisplayResult(List < float[] > sequenceData)
            {
            using(HttpClient
            client = new
            HttpClient())
            ...
            var
            response = await client.PostAsync($"{baseURL}/translate", content);
